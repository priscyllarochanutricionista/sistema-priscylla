document.addEventListener('DOMContentLoaded', () => {
    ...
    const telas = {
        ...
        'receitas': document.getElementById('tela-receitas'), 
        'despesas': document.getElementById('tela-despesas'),
    ...
    // ==========================================
    // 4. CENTRAL DE RELATÓRIOS MULTI-TIPO
    // ==========================================
        ...
        formFiltro.addEventListener('submit', async (e) => {
            ...
            try {
                ...
                if(tipoRelatorio === 'faturamento' || tipoRelatorio === 'receitas' || tipoRelatorio === 'despesas') {
                    ...
                    let dados = lancamentos.filter(l => { 
                        const d = String(l.Data).split('T')[0]; 
                    ...
                    if(dados.length === 0) {
                        tbody.innerHTML = `<tr><td colspan="4" class="text-center">Nenhum registro financeiro encontrado neste período.</td></tr>`;
                        ...
                        if(tipoRelatorio === 'faturamento') {
                            resumoHTML = `<tr style="background-color: #f8fafc; font-weight: bold;"><td colspan="3" class="text-right">LUCRO DO PERÍODO:</td><td class="${(totRec - totDes) >= 0 ? 'text-success' : 'text-danger'}">${formatarMoeda(totRec - totDes)}</td></tr>`;
                        } else if(tipoRelatorio === 'receitas') {
                            resumoHTML = `<tr style="background-color: #f0fdf4; font-weight: bold;"><td colspan="3" class="text-right">TOTAL DE ENTRADAS:</td><td class="text-success">${formatarMoeda(totRec)}</td></tr>`;
                        } else {
                            resumoHTML = `<tr style="background-color: #fef2f2; font-weight: bold;"><td colspan="3" class="text-right">TOTAL DE SAÍDAS:</td><td class="text-danger">${formatarMoeda(totDes)}</td></tr>`;
                        }
                        tbody.innerHTML += resumoHTML;
                        btnPdf.style.display = 'inline-flex';
                    }
                }
                
                // 4.2 RELATÓRIO DE AGENDAMENTOS
                    ...
                    let dados = agendaCompleta.filter(a => {
                        const d = String(a.Data_Hora).split('T')[0];
                        return d >= dtInicio && d <= dtFim && String(a.Nome_Paciente).trim() !== '';
                    });
                    ...
                    let realizados = 0;
                    if(dados.length === 0) {
                        tbody.innerHTML = `<tr><td colspan="4" class="text-center">Nenhuma consulta agendada neste período.</td></tr>`;
                        btnPdf.style.display = 'none';
                    } else {
                        dados.forEach(a => {
                            const statusL = String(a.Status_Presenca).toLowerCase();
                            if(statusL.includes('realizado')) realizados++;
                            tbody.innerHTML += `<tr><td>${formatarDataHora(a.Data_Hora)}</td><td>${a.Nome_Paciente}</td><td>${a.Tipo_Consulta}</td><td>${a.Status_Presenca}</td></tr>`;
                        });
                        tbody.innerHTML += `<tr style="background-color: #f8fafc; font-weight: bold;"><td colspan="3" class="text-right">TOTAL DE PACIENTES ATENDIDOS:</td><td class="text-primary">${realizados} de ${dados.length}</td></tr>`;
                        btnPdf.style.display = 'inline-flex';
                    }
                }

                // 4.3 RELATÓRIO DE DESEMPENHO (KPIs)
                    ...
                    lancamentos.forEach(l => {
                        ...
                        if(d >= dtInicio && d <= dtFim) {
                            if(l.Tipo === 'Receita') totRec += (parseFloat(l.Valor) || 0);
                            else if (l.Tipo === 'Despesa') totDes += (parseFloat(l.Valor) || 0);
                        }
                    });

                    agenda.forEach(a => {
                        const d = String(a.Data_Hora).split('T')[0];
                        if(d >= dtInicio && d <= dtFim && String(a.Nome_Paciente).trim() !== '') {
                            agendados++;
                            const statusL = String(a.Status_Presenca).toLowerCase();
                            const tipoL = String(a.Tipo_Consulta).toLowerCase();
                            if(statusL.includes('realizado')) realizados++;
                            if(tipoL.includes('1ª') || tipoL.includes('1a')) prim++;
                            if(tipoL.includes('retorno')) ret++;
                        }
                    });

                    tbody.innerHTML = `
                        <tr><td><strong>Faturamento Bruto</strong></td><td class="text-success">${formatarMoeda(totRec)}</td></tr>
                        <tr><td><strong>Custos / Despesas</strong></td><td class="text-danger">${formatarMoeda(totDes)}</td></tr>
                        <tr style="background-color: #f8fafc;"><td><strong>LUCRO LÍQUIDO</strong></td><td style="font-weight:bold;">${formatarMoeda(totRec - totDes)}</td></tr>
                        <tr><td>Total de Consultas Marcadas</td><td>${agendados}</td></tr>
                        <tr><td>Consultas Realizadas (Conversão)</td><td class="text-success">${realizados} (${agendados > 0 ? Math.round((realizados/agendados)*100) : 0}%)</td></tr>
                        <tr><td>Novos Pacientes (1ª Consulta)</td><td class="text-primary">${prim}</td></tr>
                        <tr><td>Retornos</td><td>${ret}</td></tr>
                    `;
                    btnPdf.style.display = 'inline-flex';
                }
            } catch (e) {
    ...
    // Geração Inteligente do PDF
    document.getElementById('btn-gerar-pdf').addEventListener('click', () => {
        const { jsPDF } = window.jspdf; const doc = new jsPDF();
        const select = document.getElementById('tipo-relatorio');
        const nomeRelatorio = select.options[select.selectedIndex].text;
        
        doc.setFontSize(16); doc.text(`Relatório: ${nomeRelatorio}`, 14, 20);
...
let graficoB = null; let graficoC = null; let graficoA = null;
...
function formatarData(valor) {
    if (!valor) return '-'; 
    try { const d = String(valor).split('T')[0].split('-'); if (d.length >= 3) { let ano = d[d.length-3].replace('+',''); if(ano.length>4) ano=ano.substring(ano.length-4); return `${d[d.length-1]}/${d[d.length-2]}/${ano}`; } return valor; } catch(e) { return valor; }
}
function formatarDataHora(valorISO) {
    if(!valorISO) return '-'; 
    try { const valStr = String(valorISO); const partes = valStr.split('T'); if(partes.length === 1) return formatarData(partes[0]); return `${formatarData(partes[0])} às ${partes[1].substring(0,5)}`; } catch(e) { return valorISO; }
}
...
// ==========================================
// CARREGAMENTO DE DASHBOARD E AGENDA
// ==========================================
...
async function carregarAgenda() {
    ...
    let totalMes = 0, prim = 0, ret = 0, real = 0, agendados = 0;
    ...
    agenda.forEach(a => {
        ...
        if (dataAg.startsWith(anoMesAtual)) {
            ...
            if(a.Status_Presenca === 'Aguardando') agendados++;
        }
    });
    ...
    const ctxA = document.getElementById('graficoAgendaStatus');
    if (ctxA) {
        graficoA = new Chart(ctxA, { type: 'pie', data: { labels: ['Aguardando', 'Realizado'], datasets: [{ data: [agendados, real], backgroundColor: ['#f59e0b', '#10b981'], borderWidth: 0 }] }, options: { responsive: true, maintainAspectRatio: false } });
    }
}
...
window.excluirRegistro = async function(tabela, id) {
    ...
    if (res.status === 'sucesso') {
        ...
        if(tabela === 'Lancamentos') carregarPainelFinanceiro(); if(tabela === 'Categorias') carregarCategorias(); if(tabela === 'Pacientes') carregarPacientes(); if(tabela === 'Agenda') carregarAgenda();
    } else showToast('Erro: ' + res.mensagem, 'erro');
}
