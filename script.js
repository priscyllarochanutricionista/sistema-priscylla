...
async function carregarAgenda() {
    ...
    agenda.forEach(a => { if(a.Tipo_Consulta === '1ª Consulta') prim++; if(a.Tipo_Consulta === 'Retorno') ret++; if(a.Status_Presenca === 'Realizado') real++; });
    const kpiTot = document.getElementById('kpi-agenda-total');
    if(kpiTot) { kpiTot.textContent = agenda.length; document.getElementById('kpi-agenda-tipos').textContent = `${prim} / ${ret}`; document.getElementById('kpi-agenda-realizados').textContent = real; }

    renderizarTabela('tbody-agenda', agenda.reverse(), a => `<td><strong>${formatarDataHora(a.Data_Hora)}</strong></td><td>${a.Nome_Paciente}</td><td><span class="badge" style="background:#e0f2fe; color:#0284c7;">${a.Tipo_Consulta}</span></td><td><span class="text-${a.Status_Presenca==='Realizado'?'success':'warning'}"><i class="ph ph-${a.Status_Presenca==='Realizado'?'check':'clock'}"></i> ${a.Status_Presenca}</span></td><td class="text-center">${botoesAcao('Agenda', a.ID)}</td>`);
}
...
// ==========================================
// AÇÕES PRO (EXCLUIR E EDITAR MODAL)
// ==========================================
